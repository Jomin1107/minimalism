						...
						<c:forEach items="${orderInfo}" var="oi" varStatus="status">
							<tr>
								...
								<td class="td_width_0"><!-- 주문정보 가져오기-->
									...
									<input type="hidden" id="cart_number${status.index}" value="${oi.cart_number}">
								</td>
								...
								<td class="td_width_3"><!-- 상품정보 -->
									<a href="/productDetail/${oi.product_number}">${oi.product_name}</a><br>
									${oi.product_color}<!-- 색상 -->
								</td>
								<td class="td_width_2 table_text_align_center"><!-- 수량 -->
									<div class="table_text_align_center quantity_div">
										<span id="numberUpDown${status.index}">${ci.product_count}</span>
									</div>
								</td>
								<td class="td_width_1"><!-- 상품구매금액 -->
									<fmt:formatNumber value="${oi.product_price}" pattern="#,###원"/>
								</td>
								</td>
								<td class="td_width_2 table_text_align_center"><!-- 주문처리상태 -->
									${oi.order_status}
								</td>
								<td class="td_width_2">-</td><!-- 취소/교환/반품 -->
								<td class="td_width_1 table_text_align_center"><!-- 선택 -->
